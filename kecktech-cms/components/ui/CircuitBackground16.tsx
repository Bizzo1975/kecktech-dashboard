...
const CircuitBackground16: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const customPathBuilders = {
      // Yellow line on right-middle of About rectangle: extends down to "Values" then bends right
      'about-right-middle-yellow': (
        ...
        // Find "Our Values" text position - try multiple selectors
        let valuesHeading = document.querySelector('section#about h3');
        if (!valuesHeading) {
          // Try finding by text content
          const allH3s = document.querySelectorAll('section#about h3');
          for (const h3 of Array.from(allH3s)) {
            if (h3.textContent?.includes('Values')) {
              valuesHeading = h3;
              break;
            }
          }
        ...
        // Calculate target Y position - extend DOWN to "Our Values" text, then bend right
        let valuesY = startY + scaledBaseLength * 12; // Default fallback
        if (valuesHeading) {
          const valuesRect = valuesHeading.getBoundingClientRect();
          const scrollY = window.scrollY || window.pageYOffset || 0;
          // Use the top of the "Our Values" text - stop 15px before it and bend right
          // Increased margin to stop a bit higher (was 12px, now 15px)
          const calculatedY = valuesRect.top + scrollY - 15; // Stop 15px before the text
          
          // Always use calculated Y if it's below startY - we need to reach the text
          if (calculatedY > startY) {
            valuesY = calculatedY;
          } else {
            // If text is somehow above, use default
            valuesY = startY + scaledBaseLength * 12;
          }
        } else {
          // If we can't find the text, use default
          valuesY = startY + scaledBaseLength * 12;
        }
        
        // Ensure it extends down far enough to reach the text area
        const minExtension = startY + scaledBaseLength * 6;
        if (valuesY < minExtension) {
          valuesY = minExtension;
        }
        
        // Cap to prevent going too far (like to contact section)
        const maxExtension = startY + scaledBaseLength * 18;
        if (valuesY > maxExtension) {
          valuesY = maxExtension;
        }
        
        // Create path: right -> down to Values -> right
        // Structure must match createCleanBendPath format exactly for proper arcTo rendering
        // createCleanBendPath structure: [start, straightEnd, corner, end]
        // For multiple bends, we need: [start, straightEnd1, corner1, midPoint, corner2, end]
        const points: { x: number; y: number }[] = [];
        
        // 1. Start point (right side of rectangle, middle position)
        points.push({ x: startX, y: startY });
        
        // 2. Go straight RIGHT (beforeBend) - this is the straightEnd for first bend
        const rightEndX = startX + line.beforeBend;
        points.push({ x: rightEndX, y: startY });
        
        // 3. First corner point for right->down turn
        // Following createCleanBendPath: direction='right', turnDirection='right' (down)
        // corner is at (rightEndX, startY + cornerRadius)
        const corner1X = rightEndX;
        const corner1Y = startY + scaledCornerRadius;
        points.push({ x: corner1X, y: corner1Y });
        
        // 4. Go DOWN to Values - this is the straightEnd for second bend
        // Stop cornerRadius before the second corner
        const downEndY = valuesY - scaledCornerRadius;
        points.push({ x: corner1X, y: downEndY });
        
        // 5. Second corner point for down->right turn
        // Following createCleanBendPath: direction='down', turnDirection='right' (right)
        // corner is at (downEndX, downEndY + cornerRadius) = (corner1X, valuesY)
        const corner2X = corner1X;
        const corner2Y = valuesY;
        points.push({ x: corner2X, y: corner2Y });
        
        // 6. Final end point (go right toward right side of screen)
        const finalEndX = corner2X + scaledBaseLength * 2;
        const finalEndY = corner2Y;
        points.push({ x: finalEndX, y: finalEndY });
        
        return {
          points,
          endCircle: { x: finalEndX, y: finalEndY },
          color: 'yellow',
          cornerRadius: scaledCornerRadius, // Use same radius as all other lines
        };
      },
    ...
    const drawCircuit = () => {
      ...
      configs.forEach(config => {
        ...
        let baseX = headingRect.left + headingRect.width / 2;
        let baseY = headingRect.top + headingRect.height / 2 + scrollY;
        
        // Handle special positioning for "Why Kecktech?" and "Contact Us" (they have text offsets)
        if (config.id === 'why' || config.id === 'contact') {
          let textOffsetX = 0;
          let parentElement = element.parentElement;
          while (parentElement && parentElement !== document.body) {
            const inlineStyle = parentElement.getAttribute('style');
            if (inlineStyle) {
              if (config.id === 'why' && inlineStyle.includes('right:')) {
                const match = inlineStyle.match(/right:\s*([-\d.]+)px/);
                if (match) {
                  textOffsetX = -parseFloat(match[1]) || 0;
                  break;
                }
              } else if (config.id === 'contact' && inlineStyle.includes('left:')) {
                const match = inlineStyle.match(/left:\s*([-\d.]+)px/);
                if (match) {
                  textOffsetX = parseFloat(match[1]) || 0;
                  break;
                }
          ...
          baseX = baseX - textOffsetX;
        }
        
        // Apply manual offsets
        const rectX = baseX + config.manualOffsetX;
        const rectY = baseY + config.manualOffsetY;
        
        // Draw the rectangle
        drawCircuitPatternAt(ctx, rectX, rectY, config);
      });
    ...
    const createCircuitPatternAt = (
      ...
      const scaledProcessorWidth = processorWidth * scale * widthMultiplier;
      const scaledProcessorHeight = processorHeight * scale;
      const scaledBaseLength = baseLength * scale;
      const scaledCornerRadius = cornerRadius * scale;
    ...
    const drawCircuitPatternAt = (
      ...
      ctx.fillStyle = "rgba(100, 130, 180, 0.28)";
      ctx.fillRect(targetX - scaledHalfWidth, targetY - scaledHalfHeight, scaledProcessorWidth, scaledProcessorHeight);
      
      ctx.strokeStyle = "rgba(150, 180, 255, 0.55)";
      ctx.lineWidth = 2.5;
      ctx.strokeRect(targetX - scaledHalfWidth, targetY - scaledHalfHeight, scaledProcessorWidth, scaledProcessorHeight);
...
export default CircuitBackground16;
