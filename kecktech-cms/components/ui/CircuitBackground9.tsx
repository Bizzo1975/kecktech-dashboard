...
const CircuitBackground9: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const createRoundedPath = (
      ...
      // Determine path: could be L-shaped or straight
      const isHorizontal = Math.abs(dx) > Math.abs(dy);
      const needsTurn = Math.abs(dx) > cornerRadius && Math.abs(dy) > cornerRadius;
    ...
    const initCircuit = () => {
      ...
      const baseLength = Math.min(width, height) * 0.22;
      ...
      for (let i = 0; i < linesPerSide; i++) {
        ...
        // Vary end positions
        const offsetX = (i - linesPerSide / 2) * 40;
        const endX = startX + offsetX;
        const endY = startY - (baseLength + Math.random() * 30);
      ...
      for (let i = 0; i < linesPerSide; i++) {
        ...
        const offsetY = (i - linesPerSide / 2) * 40;
        const endX = startX + (baseLength + Math.random() * 30);
    ...
    const drawCircuit = () => {
      ...
      balls.forEach((ball) => {
        ...
        // Draw electricity ball with glow (same style as Background4)
...
export default CircuitBackground9;
