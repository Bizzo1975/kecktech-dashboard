...
foreach ($a in @($reg.'willworkforlunch.com'.primary) + @($reg.'willworkforlunch.com'.also)) { if ($a) { $aliases.Add([string]$a) } }
$aliases.Add([string]$reg.'jacob-roman.com'.primary)
$aliases.Add([string]$reg.'unclejonsitgarage.com'.primary)
# also common extras already on mailbox
@(
...
Write-Host "Sending $($unique.Count) tests to $SharedMailbox ..."

foreach ($fromAlias in $unique) {
  $subject = "[TEST] site-mail $fromAlias"
  $body = @"
Automated cutover proof.
FromAlias: $fromAlias
To: $SharedMailbox
...
"@
  ...
  try {
    Invoke-MgGraphRequest -Method POST -Uri "https://graph.microsoft.com/v1.0/users/$([uri]::EscapeDataString($SharedMailbox))/sendMail" -Body $payload -ContentType "application/json"
    Write-Host "  OK $fromAlias"
  } catch {
    Write-Warning "  FAIL $fromAlias — $($_.Exception.Message)"
  }
}

Write-Host ""
Write-Host "Open: https://outlook.office.com/mail/$SharedMailbox/"
